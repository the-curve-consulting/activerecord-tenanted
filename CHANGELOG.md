...
- PostgreSQL is now a supported database. Set the `adapter` of a tenanted configuration to `postgresql`, and put the `%{tenant}` specifier in its `database` key. Each tenant then gets a database of its own on the server, and each tenant has a connection pool of its own. A `template` key in the configuration names the database that `CREATE DATABASE` copies. A tenant name may hold letters, digits, `-`, `_` and `~`, and the whole database name must fit in the 63 bytes that PostgreSQL allows. @jamesridgway
...
- A MySQL database adapter, registered for the `mysql2` and the `trilogy` connection adapters. Each tenant gets its own database on the server, and the `%{tenant}` specifier in the database name says how the name is built. A tenant name may contain only letters, digits, `-` and `_`, which is narrower than the SQLite adapter allows, because the name becomes part of a database name and MySQL makes a directory for each database. MySQL limits a database name to 64 characters, and a longer name raises `BadTenantNameError`. The database that a tenant is being created in is marked as not ready with a `GET_LOCK` advisory lock, as the SQLite adapter does with a lock file. Ported from the work of @andrewmarkle and @QuirkQ. @jamesridgway
...
- An application that does not load Active Storage now boots. The railtie required `active_storage/service/disk_service` in every application, so an application that stores its files another way, such as with Shrine, failed with `uninitialized constant ActiveStorage::Service`. The Active Storage integration now sets itself up only when the application loads the Active Storage engine. @jamesridgway
- `create_tenant` no longer asks whether the database exists before it creates it. `create_database` is idempotent and says whether it made the database, so the question was asked twice. On a database server each question is a connection and a query. @jamesridgway
- `destroy_tenant` now works on PostgreSQL with Rails 8.1. The adapter left the statement to Rails, which writes `DROP DATABASE ... WITH (FORCE)` only from the version after 8.1. This gem keeps a connection pool for each tenant, so a session on the database being dropped is the usual case, and without `WITH (FORCE)` the server refuses the drop with `PG::ObjectInUse`. The adapter now writes the statement itself, and adds `WITH (FORCE)` on PostgreSQL 13 and later. @jamesridgway
...
- The guide has a section on choosing a database adapter. It shows a configuration for each of SQLite, MySQL and PostgreSQL, and it sets out what a tenant name may contain and how long a database name may be for each, because a tenant name is not portable between them: a tenant called `acme.co` is valid on SQLite and cannot be created on MySQL or PostgreSQL. @jamesridgway
- The guide describes the connection budget of a tenanted application on a database server. A SQLite database costs a file descriptor, but a database on a server costs a connection, and a server has a hard limit. The worst case is `max_connection_pools` × `pool` × processes, which is 250 for one process with the default settings, against the 100 connections that PostgreSQL accepts by default and the 151 that MySQL accepts. The guide says which of `max_connection_pools`, `pool` and `idle_timeout` to change, and why `pool` is usually the one. @jamesridgway

### Developer infrastructure

- The browser tests wait longer for the webdriver. A CI runner is slower, and busier, than a developer machine, and a run failed with `Net::ReadTimeout`, which is the wait for the webdriver running out rather than a fault in the application under test. @jamesridgway
- CI runs the unit tests and the integration tests against MySQL, trilogy and PostgreSQL, as well as SQLite. The SQLite jobs start no server, so they do not wait for one they never use. A `trilogy` scenario is added, because a scenario is what selects an adapter, and the registration for trilogy had none.
- `bin/db-servers` starts the test database servers. `bin/setup` calls it, and CI runs it as a step. A service container of a GitHub workflow takes docker options and not arguments for the server itself, so it cannot be given the settings that make a run fast; running the same script in both places keeps a developer and CI on servers with the same settings. @jamesridgway
- The MySQL and the PostgreSQL containers that `bin/setup` starts are tuned for a test run. The durability of the data is turned off, and the connection limit is raised. The test suite creates and drops a database for each test, and those are the operations that a server makes durable, so the settings decide how long a run takes: a PostgreSQL `DROP DATABASE` takes 1525 ms with the default settings and 17 ms without them. The PostgreSQL unit run falls from 700 seconds to 44 seconds, and the MySQL unit run from 600 seconds to 140 seconds. A container that already exists keeps the settings it was made with, so remove the containers to pick up the change. @jamesridgway
- A parallel test worker can now report a failure that carries an ActiveRecord error. A worker sends its results to the parent process with `Marshal`, an ActiveRecord error holds the connection pool it came from, a pool holds a `Monitor`, and a `Monitor` cannot be marshalled. The worker died with `no _dump_data is defined for class Monitor`, and every failure that it found was lost, so a failing parallel run had to be repeated serially to be read. A failure that cannot be sent is now replaced with one that carries the same class name, message and backtrace. @jamesridgway
- The integration test suite drops the databases of each scenario when the scenario ends. A SQLite database is a file in the app directory, which is removed with the directory, but a database on a server stays until it is dropped, so a run left behind the databases of each scenario app and of every tenant that its tests made. @jamesridgway
- The integration test suite runs with `eager_load` turned on, and not only under CI. Rails defines the attribute methods of every model when it eager loads, which reads the schema of each model, and a tenanted model has no tenant at boot. The suite therefore shows that an application boots and loads its models from the schema cache dump, with no connection to a tenant database. This now runs against MySQL and PostgreSQL as well as SQLite. @jamesridgway
